...
| `MAX_RECHECK` | `2` | Rechecks after accepted fixes (initial pass + N rechecks; the gate ALWAYS terminates). Raised from 1 to 2 (2026-09-18): with one recheck, the fix for the recheck's own finding was never verified, so two gates in a row ended red on a single unreviewed fix. |
