...
**Either way, write the gate anchor into `PLAN_FILE`** — the section `## Closing gate (after the build)` with the marker `<!-- claudex-gate: pending; due-after: <last plan step> -->`, exactly as specified in the `claudex-loop` skill (Resolution) — including what may go into the due point and that the user sees it with the sign-off question. The plan outlives this session; the build usually happens in another one that never loads this skill, and the marker is what tells it — and the `gate_reminder` hook at `git commit` — that a gate is still ahead, and after which step it is due. A plan without stages gets `due-after: build`.
